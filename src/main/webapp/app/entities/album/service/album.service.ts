...
type RestOf<T extends IAlbum | NewAlbum> = Omit<T, 'albumReleaseDate' | 'dateAddedToDB' | 'dateLastModified'> & {
...
export class AlbumService {
  ...
  protected convertDateFromClient<T extends IAlbum | NewAlbum | PartialUpdateAlbum>(album: T): RestOf<T> {
    return {
      ...album,
      albumReleaseDate: album.albumReleaseDate?.format(DATE_FORMAT) ?? null,
      dateAddedToDB: album.dateAddedToDB?.format(DATE_FORMAT) ?? null,
      dateLastModified: album.dateLastModified?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAlbum: RestAlbum): IAlbum {
    return {
      ...restAlbum,
      albumReleaseDate: restAlbum.albumReleaseDate ? dayjs(restAlbum.albumReleaseDate) : undefined,
      dateAddedToDB: restAlbum.dateAddedToDB ? dayjs(restAlbum.dateAddedToDB) : undefined,
      dateLastModified: restAlbum.dateLastModified ? dayjs(restAlbum.dateLastModified) : undefined,
    };
  }
