...
type RestOf<T extends ISong | NewSong> = Omit<T, 'songDateAddedToDB' | 'songDateLastModified'> & {
  songDateAddedToDB?: string | null;
  songDateLastModified?: string | null;
};
...
export class SongService {
  ...
  protected convertDateFromClient<T extends ISong | NewSong | PartialUpdateSong>(song: T): RestOf<T> {
    return {
      ...song,
      songDateAddedToDB: song.songDateAddedToDB?.format(DATE_FORMAT) ?? null,
      songDateLastModified: song.songDateLastModified?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSong: RestSong): ISong {
    return {
      ...restSong,
      songDateAddedToDB: restSong.songDateAddedToDB ? dayjs(restSong.songDateAddedToDB) : undefined,
      songDateLastModified: restSong.songDateLastModified ? dayjs(restSong.songDateLastModified) : undefined,
    };
  }
